...
object PdfExportManager {
    ...
    suspend fun generatePdf(context: Context, project: Project): File? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            var y = drawHeader(canvas, paint, project)
            ...
            fun ensureSpace(required: Float) {
                ...
                canvas = page.canvas
                y = drawCompactHeader(canvas, paint, project)
            }

            project.items.groupBy { it.category.ifBlank { "Other" } }.toSortedMap().forEach { (category, items) ->
                ensureSpace(54f)
                y += 18f
                drawCategoryHeader(canvas, paint, category, y)
                y += 30f
                items.forEachIndexed { index, item ->
                    val lines = wrapText(paint, item.displayName(index + 1), 330f, 11.5f)
                    val rowHeight = (lines.size.coerceAtLeast(1) * 15f) + 12f
                    ensureSpace(rowHeight)
                    y = drawItemRow(canvas, paint, lines, item, y, rowHeight)
                }
            }
            ...
            val file = File(exportDir, "PipeList_${safeFileName(project.projectName)}_${System.currentTimeMillis()}.pdf")
